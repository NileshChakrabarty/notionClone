...
const Some = () => {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100" onClick={()=>{setIsAddingNote(true)}}>
  {/* Sidebar */}
  <div className="w-64 bg-white p-6 flex flex-col shadow-md">
    {/* User Info */}
        ...
        filteredNotes.map((note, index) => (
          ...
          >
            {isRenaming === index ? (
              <input
                type="text"
                className="p-2 w-full rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none"
                value={renameInput}
    ...
    {isAddingNote && (
      <div className="mt-4">
        ...
        />
        <button
          className="mt-2 bg-blue-500 hover:bg-blue-600 text-white w-full py-2 rounded-md focus:outline-none"
          onClick={addNewNote}
        >
          Add Note
        </button>
