...
const Home = () => {
  ...
  const handleSignup = () => {
    localStorage.setItem('isLoggedIn', 'true');
    setIsLoggedIn(true);
    navigate('/some'); // Redirect after signup
    closeModal();
  };
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg w-full max-w-md relative">
            ...
            {isLogin ? (
              <LoginForm darkMode={darkMode} handleLogin={handleLogin} closeModal={closeModal} />
            ) : (
              <SignupForm darkMode={darkMode} handleSignup={handleSignup} closeModal={closeModal} />
            )}
